(function ($) {
    ...
    $(document).ready(function () {
        ...
        contentArea.on('click', '.wppob-category-item', function (e) {
            const item = $(this);
            // Hanya proses jika item bukan link produk langsung
            if (item.is('a')) {
                return;
            }
            e.preventDefault();
        ...
        function loadCategoryContent(categoryId, categoryName) {
            ...
            $.ajax({
                url: wppob_frontend_ajax.ajax_url,
                type: 'POST',
                data: {
                    action: 'wppob_get_category_content',
                    nonce: wppob_frontend_ajax.nonce,
                ...
                success: function (response) {
                    if (response.success) {
                        ...
                        contentArea.html('<p>Gagal memuat konten.</p>');
                    }
                },
                error: function () {
                    contentArea.html('<p>Terjadi kesalahan koneksi.</p>');
                },
                complete: function () {
                    loader.hide();
                }
        ...
        function renderContent(data) {
            ...
            if (data.display_mode === 'list') {
                contentArea.removeClass('grid-view').addClass('list-view');
            } else {
                contentArea.removeClass('list-view').addClass('grid-view');
            }
